...
$\sigma_{x=a}(R)$.  In pseudocode (where $R_i.x$ is the $x$-column of
relation $R_i$):
%
\begin{lstlisting}[basicstyle=\ttfamily]
def GJ(Q, tuple): # tuple is initially empty
  if Q is empty: print(tuple)
  else:
    for a in $\bigcap_{i : x \in schema(R_i)} R_i.x$:
      tuple.append(a)
      GJ(Q[a/x]) # compute Q with one fewer variable
\end{lstlisting}
%
